# DAILY-EXPENSE-CRUD-MANAGER

Description :-
Daily Expense CRUD Manager is a simple and efficient Python Project which helps individuals track there daily finance. 
The CRUD functionality i.e. Create, Read, Update and Delete, makes it easier to manage spending habits, maintain records and gain better financial awareness.

Features :-
My project contains 2 python files named :- 1. expense and 2. expense_tracker
1. expense contains constructor to initialise name, amount, category and subcategory variables
2. expense_tracker contains the operations that is to be done to organize and maintain expense records

Project contains following features :- 
1. Adds new expense records that includes name of expense, amount, category and and subcategory
   Category includes
     1. Needs
        1. Food
        2. Home
        3. Learn
     3. Wants
        1. Entertainment
        2. Shopping
        3. Others
3. View all recorded expense
4. Delete unwanted entries
5. Edit/Update any record

Tools Used :-
1. Programming Language - Python
2. Database - Microsoft Excel

Installation :-
1. Install Virtual Studio Code and Git
2. Clone the Github Repository using git clone
3. Add README.md and statement.md
4. Add required git commands to add and push files into repository
5. Run the application

<img width="638" height="277" alt="image" src="https://github.com/user-attachments/assets/b830874a-5e1e-417c-b372-4c9dde5eb0ef" />


Project Structure :-

Daily-Expense-CRUD-Manager
| ---README.md
| ---statement.md
| ---expense.py
| ---expense_tracker.py
     | ---importing Expense from expense
     | ---importing calender
     | ---importing datetime from datetime
| ---Comprehensive_Commit_History

<img width="638" height="277" alt="image" src="https://github.com/user-attachments/assets/0b6acfc2-2317-4e8d-962e-f266142df320" />


Instructions for testing :-
1. Open the application
2. Add expenses by entering
   1. Name of expense
   2. Amount
   3. Category of expense
   4. Subcategory of expense
3. View Expense List
4. View total spending and remaining budget
5. Edit or Delete entries as required



